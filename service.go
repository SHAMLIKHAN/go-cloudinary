...
func Dial(uri string) (*Service, error) {
	...
	s := &Service{
		...
		simulate:      false,
		verbose:       false,
		SecureUrl:     make(map[string]string),
	}
...
func (s *Service) walkIt(path string, info os.FileInfo, err error) error {
	...
	secureUrl, err := s.uploadFile(path, nil, false)
...
func (s *Service) UploadImage(path string, data io.Reader, prepend string) (map[string]string, error) {
	_, err := s.Upload(path, data, prepend, false, ImageType)
	return s.SecureUrl, err
}
